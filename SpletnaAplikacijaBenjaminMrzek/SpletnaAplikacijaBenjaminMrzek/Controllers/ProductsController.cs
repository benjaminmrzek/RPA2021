...
using SpletnaAplikacijaBenjaminMrzek.Models;
using Newtonsoft.Json;
using Newtonsoft.Json.Linq;

namespace SpletnaAplikacijaBenjaminMrzek.Controllers
{
    public class ProductsController : Controller
    {
        private NorthwindModel db = new NorthwindModel();
        //daj v json format
        public ActionResult JsonIzpis()
        {
            string poizvedba = "select * from Products";
            var rezultat = db.Database.SqlQuery<Products>(poizvedba);
            var rez = rezultat.ToList();
            var jsonRezultat = JsonConvert.SerializeObject(rez);
            ViewBag.Jsonrezultat = jsonRezultat;
            return View();
        }
        // GET: Products
